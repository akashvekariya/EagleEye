...
RUN apt-get update && add-apt-repository -y ppa:deadsnakes/ppa && apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y python3.11 python3.11-dev
...
# change the branch after use
RUN git clone https://github.com/akashvekariya/EagleEye.git
WORKDIR EagleEye
RUN git checkout test
RUN pip3.11 install -r requirements.txt
RUN pip3.11 install --upgrade beautifulsoup4 html5lib spry
